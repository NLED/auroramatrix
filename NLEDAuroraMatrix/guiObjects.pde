...
class guiNumberInputField
{
  ...
  if (software.mouseOverEnabled  == true) NumberInputList.add(this);
  MouseOverFlag = false;   
  ...
  void display()
  {
    ...
    if (selected == 3) stroke(colTxtHL);
    else if(mouseOver() == 3) stroke(colTxtHL,128);
    else       stroke(gui.buttonColor);
    rect(xpos+size+(size/4), ypos, fieldWidth, size); //textfield
    ...
    else if (inputMethod == 1 && selected == 3) text(label, xpos+(size*1.25), ypos+((size-(textAscent()+textDescent()))/1.5), fieldWidth, size);
    else if (inputMethod == 1) text(""+int(label), xpos+(size*1.25), ypos+((size-(textAscent()+textDescent()))/1.5), fieldWidth, size);  
    else  if (inputMethod == 2)  text(int(label)+"%", xpos+(size*1.25), ypos+((size-(textAscent()+textDescent()))/1.5), fieldWidth, size); //percentage

    strokeWeight(1);
    stroke(colButtonStroke);

    if (selected == 1) fill(colTxtHL);
    else if(mouseOver() == 1)  fill(colButtonBG, 64);    
    else fill(colButtonBG);
    rect(xpos, ypos, size, size, size/4);  //rounded corners


    if (selected == 2) fill(colTxtHL);
    else if(mouseOver() == 2)  fill(colButtonBG, 64);
    else fill(colButtonBG);
    rect(xpos+(size*1.5)+fieldWidth, ypos, size, size, size/4);  
...
class guiSliderBar
{
  ...
  void setValue(int passedVal)
  {
    ...
    value = passedVal;
    method(callBackStr); //run callback to update any other variabes, since setValue over rides move
  }//end setValue() 
